...
_5a0[type]("setValue",val);
        //set name
        if(type=="combogrid" && dat[name+"Name"]){
            _5a0[type]("setText",dat[name+"Name"]);
        }
}
return true;
}
...
if(_5a5.length&&_5a5[type]){
    var dis=$(_5a5)[type]("options").disabled;
    if(dis || dis=="disabled"){
        return false;
    }else{
        _5a5[type]("clear");
...
opts.onClickCell.call(_6c4,_6c5,_6c8,row[_6c8],e);
}
...
if(col && col.width && String(col.width||"").indexOf("%")>=0){
...
},onClickCell:function(_87d,_87e,_87f,e){//add e
...
opts.onClickCell.call(_914,_913,_918,row[_918],e);//add e
}
...
//不能设置时间处理
    var str = date.format("MM/dd/yyyy hh:mm:ss.S");
    $(_b88).datebox('setValue',str);
}});
}
...
//不能设置时间处理
    var str = date.format("MM/dd/yyyy hh:mm:ss.S");alert(str);
    $(this.target).datetimebox('setValue',str);
}});
if(!_bad.spinner){
